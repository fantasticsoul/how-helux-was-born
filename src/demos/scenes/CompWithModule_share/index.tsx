...
const Demo = () => (
  <Entry fns={getAtionFns(action.actions, [test, testF, reactiveF])}>
    ...
    <Comp2 />
    <Comp3 />
    <div>ctx.state.val.f {$(ctx.state.f)}</div>
    <div>ctx.state.val.g {$(ctx.state.g)}</div>
    <div>ctx.state.val.k {$(ctx.state.k)}</div>
    <div>ctx.state.val.j {$(ctx.state.j)}</div>
  </Entry>
);
