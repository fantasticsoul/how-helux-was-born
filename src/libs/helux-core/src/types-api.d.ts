 ...
 */
export function mutate<T extends SharedDict>(target: T): <A extends ReadOnlyArr = ReadOnlyArr>(fnItem: MutateFnItem<T, A>) => MutateWitness<T>;

/**
 * 作用同 mutate，为 atom 生成的对象创建一个 mutate 函数
 */
export function atomMutate<T extends any>(target: Atom<T>): <A extends ReadOnlyArr = ReadOnlyArr>(fnItem: AtomMutateFnItem<T, A>) => MutateWitness<T>;
