...
export function handleDataNode(parentDataNode: any, copyCtx: { parentMeta: DraftMeta } & AnyObject) {
  ...
  if (OP_DEL === op) {
    ...
    if (valueMeta) {
      ...
      if (keyPaths.length === 1) {
        ...
        // 存在多个路径指到当前对象，删一条路径，并将 keyPath 重新指向一个存在的路径
        delKeyPath(keyPaths, valueMeta.keyPath);
        valueMeta.keyPath = keyPaths[0];
      }
    } else {
  ...
  if (key === 'currentGroup') {
    console.log(` ---> write copy for key ${key} ver ${parentMeta.ver} id ${parentMeta.id}`);
  }
