...
import { createScopedMeta, getMayProxiedVal, getUnProxyValue, setTmp, getTmp } from './helper';
import { genMetaVer, getSafeDraftMeta, isDraft, getPrivateMeta, replaceMetaPartial, ROOT_CTX } from './meta';
...
export function buildLimuApis(options?: IInnerCreateDraftOptions) {
  ...
  const logSetExpiredValFailed = (op: string, key: string) => {
    console.warn(`${op} ${key} failed, cuase the value is an expired limu proxy data!`);
    return silenceSetTrapErr;
  };
  ...
  const limuApis = (() => {
    ...
    const limuTraps = {
      ...
      get: (parent: any, key: any) => {
        ...
        if (key === 'currentGroup') {
          console.log('getTmp() ' + metaVer, getTmp());
          console.log('getTmp()===currentVal', getTmp() === currentVal);
          console.log('getTmp()===parent.currentGroup', getTmp() === parent.currentGroup);
          console.log(
            ` ---> get meta for key ${key} ver ${metaVer} id ${parentMeta?.id || '--'} mapkey`,
      ...
      set: (parent: any, key: any, inputValue: any) => {
        ...
        if (isDraft(inputValue)) {
          ...
          if (isInSameScope(inputValue, metaVer)) {
            ...
            pushKeyPath(valueMeta.keyPaths, newKeyPath);
          } else {
          ...
          pushKeyPath(curVerMeta.keyPaths, newKeyPath);
          setValByKeyPaths(rootSelf, curVerMeta.keyPaths, curVerMeta.self);

          mayNewNode = curVerMeta.keyPaths.length === 1;
          setTmp(curVerMeta.self);
          console.log('setTmp ' + metaVer, curVerMeta.self);
