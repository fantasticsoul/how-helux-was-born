...
export type LoadingStatus = { loading: boolean, err: Error | null };
...
export interface ISharedCtx<T = SharedState, O extends ICreateOptions<T> = ICreateOptions<T>> {
  ...
  action: <A extends any[] = any[]>(fn: ActionFnDef<A, T>, desc?: string) => Action<A, T>;
  asyncAction: <A extends any[] = any[]>(fn: AsyncActionFnDef<A, T>, desc?: string) => AsyncAction<A, T>;
  state: SharedDict<T>;
  setState: SetState<T>;
  ...
  useLoading: () => [Ext<GetLoadingType<T, O>, LoadingStatus>, SetState<LoadingState>, IRenderInfo];
}

export interface IAtomCtx<T = any, O extends IAtomCreateOptions<T> = IAtomCreateOptions<T>> {
  call: AtomCall<T>;
  asyncCall: AtomAsyncCall<T>;
  action: <A extends any[] = any[]>(fn: AtomActionFnDef<A, T>, desc?: string) => AtomAction<A, T>;
  asyncAction: <A extends any[] = any[]>(fn: AtomAsyncActionFnDef<A, T>, desc?: string) => AtomAsyncAction<A, T>;
  state: Atom<T>;
  setState: SetAtom<T>;
  ...
  useLoading: () => [Ext<GetAtomLoadingType<T, O>, LoadingStatus>, SetState<LoadingState>, IRenderInfo];
}
