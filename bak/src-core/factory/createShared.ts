...
import { createAction, createAsyncAction, createAtomAction, createAtomAsyncAction } from './createAction';
...
export function createSharedLogic(innerOptions: IInnerOptions, createOptions?: any): any {
  ...
  const actionCreator = forAtom ? createAtomAction(state) : createAction(state);
  const asyncActionCreator = forAtom ? createAtomAsyncAction(state) : createAsyncAction(state);
