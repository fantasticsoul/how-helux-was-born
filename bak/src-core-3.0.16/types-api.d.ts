 ...
 */
export function share<T = Dict>(
  rawState: T | (() => T),
  strBoolOrCreateOptions?: ICreateOptionsType<T>,
): readonly [SharedDict<T>, SetState<T>, ISharedCtx<T>];
...
): readonly [Atom<T>, SetAtom<T>, IAtomCtx<T>];
 ...
 * shareState 返回对象{state,setState,call,asyncCall}
 */
export function shareState<T = Dict>(
  rawState: T | (() => T),
  strBoolOrCreateOptions?: ICreateOptionsType<T>,
): ISharedCtx<T>;
...
export function defineMutateRules<T = SharedState>(options: IDefMutateRuleOptions<T>);
