...
import type { AsyncSetState, IHeluxParams, IInnerSetStateOptions, KeyInsKeysDict, SetAtom, SetState, SharedState, InnerSetState } from '../../types';
...
import { parseRules, parseSetOptions } from './parse';
import { createSyncFnBuilder, createSyncerBuilder } from './sync';

export function mapSharedToInternal(sharedState: SharedState, heluxParams: IHeluxParams) {
  const { createOptions, rawState, sharedKey } = heluxParams;
  const { deep, forAtom, mutate, watch, loc, changeFn } = createOptions;
