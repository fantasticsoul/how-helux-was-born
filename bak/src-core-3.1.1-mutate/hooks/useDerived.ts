...
export function useAtomDerived<T = any>(resultOrFn: DerivedAtom<T> | DeriveAtomFn<T>) {
  const fnCtx = useDerivedLogic({ fn: resultOrFn, forAtom: true });
  return getAtomTuple<T>(fnCtx);
}

export function useAtomDerivedAsync<T = any, D = any[]>(options: IDeriveAtomAsyncOptions<T, D>) {
