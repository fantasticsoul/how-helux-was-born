...
import { SINGLE_MUTATE, LOADING_MODE, STATE_TYPE, STOP_DEPTH } from '../../consts';
...
function parseMutate(mutate?: IInnerCreateOptions['mutate'] | null) {
  ...
  const handleItem = (item: MutateFnItem | MutateFn, inputDesc?: string) => {
    if (isFn(item)) {
      ...
      mutateFns.push({ fn: item, deps: noop, desc, task: null });
    } else if (isObj(item)) {
      if (item.fn === noop) return;
      const desc = parseDesc(inputDesc, item.desc);
      const { fn = noop, deps = noop, task = null } = item;
