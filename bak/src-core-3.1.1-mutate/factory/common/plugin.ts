 ...
 * 发射数据已变更事件到已安装插件
 */
export function emitDataChanged(internal: TInternal, desc: string) {
  const { bus } = getRootCtx();
  if (bus.canEmit(ON_DATA_CHANGE)) {
    const { sharedKey, moduleName, rawStateSnap: snap, usefulName } = internal;
    console.log('emitDataChanged', moduleName, desc);
    const descVar = desc || 'setState';
    bus.emit(ON_DATA_CHANGE, { snap, sharedKey, moduleName, type: `${usefulName}/${descVar}` });
  }
}
