...
export function runMutateFnItem(sharedState: SharedState, inputDesc?: string, forTask?: boolean) {
  checkShared(sharedState, 'runMutateFn');
...
export function runMutateFnTask(sharedState: SharedState, desc?: string) {
  checkShared(sharedState, 'runMutateFnTask');
