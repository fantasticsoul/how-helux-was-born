...
export function callAsyncMutateFnLogic<T = SharedState>(targetState: T, options: ICallAsyncMutateFnLogicOptions) {
  ...
  const { before, setStateImpl, forAtom, loadingCtx } = internal;
  ...
  const changeLoading = (loading: boolean, status: boolean, err: any) => {
    if (loadingCtx && desc) {
      loadingCtx.setState((draft: any) => { draft[desc] = { loading, status, err } });
    }
  };
  ...
  changeLoading(true, true, null);
  Promise.resolve(task(...args)).then(() => {
    changeLoading(false, true, null);
  }).catch((err) => {
    changeLoading(false, false, err);
  }).finally(() => {
